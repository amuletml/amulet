do
  ...
  local writeln = print
  local function _dollardApplicativeaqw(tmp)
    return {
  ...
  local function _dollarshow(bjb, x)
    if x.__tag == "Nil" then return "Nil" end
    local tmp = x[1]
    return bjb(tmp._1) .. " :: " .. _dollarshow(bjb, tmp._2)
  end
  local function _dollartraverse(cdi, tmp, k, x)
  ...
  local function _dollar_d7(ckt, x, ys)
  ...
  writeln(_dollarshow(function(x)
    return tostring(x)
  end, _dollartraverse(nil, _dollardApplicativeaqw({
