...
instance Ord (Var p) => Substitutable p (Type p) where
  ...
  apply _ (TyCon x a) = TyCon x a
  apply _ (TyLit a) = TyLit a
  apply _ (TySkol x) = TySkol x
  apply _ (TyPromotedCon x a) = TyPromotedCon x a
  apply s (TyWildcard v) = mkWildTy (apply s <$> v)
  apply _ TyType = TyType
  apply s t@(TyVar v _) = Map.findWithDefault t v s
...
tyVarOcc (TyVar v _) = singletonOcc v
...
nominalTvs (TyVar v _) = Set.singleton v
